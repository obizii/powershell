...
# Change the SKU and Enabled Plans to reflect the services you wish to enable in your tenant users.
$SKU = "ENTERPRISEPACK_GOV"
$enabledPlans = @("POWERAPPS_O365_P2_GOV","SHAREPOINTENTERPRISE_GOV","FORMS_GOV_E3","STREAM_O365_E3_GOV","TEAMS_GOV","PROJECTWORKMANAGEMENT_GOV","RMS_S_ENTERPRISE_GOV","MYANALYTICS_P2_GOV","EXCHANGE_S_ENTERPRISE_GOV","SHAREPOINTWAC_GOV","FLOW_O365_P2_GOV","OFFICESUBSCRIPTION_GOV","MICROSOFTBOOKINGS")

# Setting script version variable
$scriptVersion="Assign Licenses - GCC G3 (version 1.0)"
...
foreach ($user in $users) {
    ...
    # Assign usage location US if field UsageLocation empty
    ...
    if($userObject.UsageLocation -eq $null){
        try{
            ...
            Write-Host "Usage Location for user $($user.UserPrincipalName) was not set. Added Usage Location US to the user!" -ForegroundColor "Yellow"
        }catch{
